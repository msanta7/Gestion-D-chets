...
public class DashConducteurController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        System.out.println("=== INITIALISATION DashConducteurController ===");
    ...
    private void debugDatabaseConnection() {
        try {
            System.out.println("=== DEBUG CONNEXION BASE DE DONNÉES ===");
            Connection conn = Database.connectDB();
            if (conn != null) {
                System.out.println("✅ Connexion réussie à la base de données");

                // Tester si la table INTERVENTION existe
                String query = "SELECT COUNT(*) as total FROM INTERVENTION";
                PreparedStatement pst = conn.prepareStatement(query);
                ResultSet rs = pst.executeQuery();
                if (rs.next()) {
                    System.out.println("Total interventions dans la base: " + rs.getInt("total"));
                }

                // Vérifier le conducteur
                System.out.println("ID Conducteur connecté: " + idConducteurConnecte);
    ...
    private void setupTableView() {
        // Configurer les colonnes - VERIFIEZ CES NOMS
        idIntervention.setCellValueFactory(new PropertyValueFactory<>("idIntervention"));
        datePlanification.setCellValueFactory(new PropertyValueFactory<>("datePlanificationFormatted")); // Notez "Formatted"
        adresseSignalement.setCellValueFactory(new PropertyValueFactory<>("adresseSignalement"));
        dateRealisation.setCellValueFactory(new PropertyValueFactory<>("dateRealisationFormatted")); // Notez "Formatted"
        statu.setCellValueFactory(new PropertyValueFactory<>("statutFormatted")); // Notez "Formatted"
        ...
        // DEBUG
        System.out.println("TableView configurée");
        System.out.println("Colonnes: " + interventionsTable.getColumns().size());
        System.out.println("Colonne date: " + datePlanification.getCellValueFactory());
    }
            ...
            """;
                ...
                while (rs.next()) {
                    ...
                    if (planifTimestamp != null) {
                        LocalDateTime datePlanif = planifTimestamp.toLocalDateTime();
                        intervention.setDatePlanification(datePlanif);

                        // DEBUG
                        System.out.println("Chargement - Date planification: " + datePlanif);
                        System.out.println("Formaté: " + datePlanif.format(DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm")));
                    }
                ...
                System.out.println("=== LISTE INTERVENTIONS CHARGÉES ===");
                for (Intervention i : interventionsList) {
                    System.out.println("ID: " + i.getIdIntervention());
                    System.out.println("Date Planif: " + (i.getDatePlanification() != null ? i.getDatePlanification() : "null"));
                    System.out.println("Formaté: " + i.getDatePlanificationFormatted());
                    System.out.println("---");
                }
